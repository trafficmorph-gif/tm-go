...
```
go get github.com/trafficmorph-gif/tm-go@v0.1.6
```
...
|---|---|---|---|
| `WithBaseURL` | `TM_BASE_URL` | none — must be set explicitly | Point at your TrafficMorph install (`http://localhost:8080` for local dev, or your hosted URL). Calling `tm.New()` without either `WithBaseURL(...)` or `$TM_BASE_URL` is a constructor error. Either spelling (with or without trailing slash) works — the SDK normalizes to a trailing-slash form so path-prefixed deployments behind reverse proxies (`https://host/proxy-prefix`) keep their prefix during URL resolution. Option and env values flow through the same validate-and-normalize pipeline, so they produce identical results for the same logical input. **Rejected at construction time**: missing scheme (`localhost:8080`), non-http(s) scheme (`ftp://x`), missing host (`https://`), query string (`https://x/?q=1`), or fragment (`https://x/#frag`) — catching these upfront beats failing late with opaque transport errors. Query strings and fragments are refused because they belong on per-request URLs, not the deployment root. **Percent-encoded path segments are preserved verbatim**: `https://host/a%2Fb` stays `/a%2Fb`, not `/a/b` (per RFC 3986 those are semantically different paths — one segment vs. two). |
